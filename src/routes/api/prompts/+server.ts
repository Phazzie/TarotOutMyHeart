...
Cards to generate (0–21): ${MAJOR_ARCANA_NAMES.join(', ')}`,
  ...
  try {
    ...
    const rawPrompts = JSON.parse(jsonMatch[0]) as Array<{
      cardNumber: number;
      cardName: string;
    ...
    const prompts = rawPrompts.map((p) => ({
      ...
      cardName: p.cardName,
      traditionalMeaning:
        MAJOR_ARCANA_MEANINGS[p.cardNumber as keyof typeof MAJOR_ARCANA_MEANINGS] ?? '',
    ...
    return json({ success: false, error: { code: 'API_ERROR', message } }, { status: 502 });
  }
};

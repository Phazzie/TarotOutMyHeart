...
<div class="deck-gallery">
	...
	{#if error}
		<div class="error-container" role="alert">
			...
			<button onclick={initializeDisplay} class="retry-button">
				Retry
			</button>
		</div>
	{/if}
		...
		<div class="card-grid" role="grid" aria-label="Tarot card gallery">
			{#each visibleCards as displayCard (displayCard.card.id)}
				<div
					...
					onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && openLightbox(displayCard.card.cardNumber)}
	...
	{#if !loading && !error && displayCards.length > 0 && visibleCount === 0}
		<div class="no-results">
			...
			<button onclick={clearFilter} class="clear-filters-button">
				Clear Filters
			</button>
		</div>
	{/if}
